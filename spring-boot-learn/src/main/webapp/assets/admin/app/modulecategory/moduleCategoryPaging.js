...
function buildReqParam(){
    var requestParam = {date:new Date().getTime()}
        setRequestParamByName(requestParam,'moduleCategoryNameFirst')
...
function  getItemPage(){
    ...
    var template = []
        ...
        template.push('<td>{moduleCategoryName}</td>')
        template.push('<td>{sortNum}</td>')
        template.push('<td>{domain}</td>')

        template.push('<td>')
        if(canUpdate=='true'){
            template.push('<a class="btn btn-sm btn-info" href="admin/module_category/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">编辑</a>')
        }else{
            template.push('<a class="btn btn-sm btn-info" href="admin/module_category/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">查看</a>')
        }
        if(canDelete=='true'){
            template.push('<a class="btn btn-sm btn-danger" href="javascript:{}" onclick="deleteItem(\'module_category\',\'{id}\')">删除</a>')
        }
        template.push('</td>')
        template.push('</tr>')
...
function exportFile(exportFile){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/module_category/export/'+exportFile+'/'+start+'/'+limit;
