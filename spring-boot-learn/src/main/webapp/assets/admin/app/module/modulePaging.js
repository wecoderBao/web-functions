...
function buildReqParam(){
    var requestParam = {date:new Date().getTime()}
        setRequestParamByName(requestParam,'moduleNameFirst')
        setRequestParamByName(requestParam,'moduleCategoryIdFirst')
        setRequestParamByName(requestParam,'moduleTitleFirst')
...
function  getItemPage(){
    ...
    var template = []
        ...
        template.push('<td>{moduleName}</td>')
        template.push('<td>{moduleUrl}</td>')
        template.push('<td><a title="点击查看详情" target="_blank" onclick="openUrl(this,\'admin/module_category/detail_param?id={moduleCategoryId}\')"> {moduleCategoryIdForeignShowValue}</a></td>')
        template.push('<td>{sortNum}</td>')
        template.push('<td>{moduleTitle}</td>')
        template.push('<td>{target}</td>')
        template.push('<td>{ifShow}</td>')

        template.push('<td>')
        if(canUpdate=='true'){
            template.push('<a class="btn btn-sm btn-info" href="admin/module/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">编辑</a>')
        }else{
            template.push('<a class="btn btn-sm btn-info" href="admin/module/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">查看</a>')
        }
        if(canDelete=='true'){
            template.push('<a class="btn btn-sm btn-danger" href="javascript:{}" onclick="deleteItem(\'module\',\'{id}\')">删除</a>')
        }
        template.push('</td>')
        template.push('</tr>')
            ...
            $.each(data.info, function(index, val){
                ...
                if(index == "pageList"){
                    ...
                    $.each(val, function(index1, value){
                        ...
                        value.CTX = CTX;
                        //自定义输出
                        var ifShowMap = {"":"全部","false":"否","true":"是"}
                        value.ifShow = buildSelect(ifShowMap,value.ifShow,'module','ifShow','id')
                        //聚焦
...
function exportFile(exportFile){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/module/export/'+exportFile+'/'+start+'/'+limit;
