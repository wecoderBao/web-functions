...
function buildReqParam(){
    var requestParam = {date:new Date().getTime()}
        ...
        setRequestParamByName(requestParam,'cardAwardIdFirst')
...
function  getItemPage(){
    ...
    var template = []
        ...
        template.push('<td><a title="点击查看详情" target="_blank" onclick="openUrl(this,\'admin/card_award/detail_param?id={cardAwardId}\')"> {cardAwardIdForeignShowValue}</a></td>')
        template.push('<td>{weight}</td>')

        template.push('<td>')
        if(canUpdate=='true'){
            template.push('<a class="btn btn-sm btn-info" href="admin/card_award_child/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">编辑</a>')
        }else{
            template.push('<a class="btn btn-sm btn-info" href="admin/card_award_child/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">查看</a>')
        }
        if(canDelete=='true'){
            template.push('<a class="btn btn-sm btn-danger" href="javascript:{}" onclick="deleteItem(\'card_award_child\',\'{id}\')">删除</a>')
        }
        template.push('</td>')
        template.push('</tr>')
    ...
    var url ='admin/card_award_child/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
...
function exportFile(exportFile){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/card_award_child/export/'+exportFile+'/'+start+'/'+limit;
