...
function  getItemPage(){
    ...
    var template = []
        ...
        template.push('<td>{newestUserId}</td>')
        template.push('<td>{randSendCardOnePrizeCode}</td>')
        template.push('<td>{oneWeight}</td>')
        template.push('<td>{randSendCardTwoPrizeCode}</td>')
        template.push('<td>{twoWeight}</td>')
        template.push('<td>{randSendCardThreePrizeCode}</td>')
        template.push('<td>{threeWeight}</td>')

        template.push('<td>')
        if(canUpdate=='true'){
            template.push('<a class="btn btn-sm btn-info" href="admin/card_old_la_new_send_config/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">编辑</a>')
        }else{
            template.push('<a class="btn btn-sm btn-info" href="admin/card_old_la_new_send_config/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">查看</a>')
        }
        if(canDelete=='true'){
            template.push('<a class="btn btn-sm btn-danger" href="javascript:{}" onclick="deleteItem(\'card_old_la_new_send_config\',\'{id}\')">删除</a>')
        }
        template.push('</td>')
        template.push('</tr>')
    ...
    var url ='admin/card_old_la_new_send_config/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
...
function exportFile(exportFile){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/card_old_la_new_send_config/export/'+exportFile+'/'+start+'/'+limit;
