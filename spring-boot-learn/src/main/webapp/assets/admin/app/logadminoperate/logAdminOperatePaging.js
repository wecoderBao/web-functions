...
function buildReqParam(){
    var requestParam = {date:new Date().getTime()}
        ...
        setRequestParamByName(requestParam,'urlFirst')
        setRequestParamByName(requestParam,'formBodyFirst')
...
function  getItemPage(){
    ...
    var template = []
        ...
        template.push('<td>{adminUserId}</td>')
        template.push('<td>{userName}</td>')
        template.push('<td>{url}</td>')
        template.push('<td>{formBody}</td>')
        template.push('<td>{createTime}</td>')

        template.push('<td>')
        if(canUpdate=='true'){
            template.push('<a class="btn btn-sm btn-info" href="admin/log_admin_operate/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">编辑</a>')
        }else{
            template.push('<a class="btn btn-sm btn-info" href="admin/log_admin_operate/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">查看</a>')
        }
        if(canDelete=='true'){
            template.push('<a class="btn btn-sm btn-danger" href="javascript:{}" onclick="deleteItem(\'log_admin_operate\',\'{id}\')">删除</a>')
        }
        template.push('</td>')
        template.push('</tr>')
    ...
    var url ='admin/log_admin_operate/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
...
function exportFile(exportFile){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/log_admin_operate/export/'+exportFile+'/'+start+'/'+limit;
