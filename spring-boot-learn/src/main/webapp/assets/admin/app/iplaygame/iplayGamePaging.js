...
function buildReqParam(){
    var requestParam = {date:new Date().getTime()}
        setRequestParamByName(requestParam,'gameNameFirst')
        setRequestParamByName(requestParam,'freshTypeFirst')
        setRequestParamByName(requestParam,'gameTypeIdFirst')
...
function  getItemPage(){
    ...
    var template = []
        ...
        template.push('<td>{linkUrl}</td>')
        template.push('<td>{freshType}</td>')
        template.push('<td><a title="点击查看详情" target="_blank" onclick="openUrl(this,\'admin/iplay_game_type/detail_param?id={gameTypeId}\')"> {gameTypeIdForeignShowValue}</a></td>')
        template.push('<td>{sortNum}</td>')
        template.push('<td>{gameName}</td>')

        template.push('<td>')
        if(canUpdate=='true'){
            template.push('<a class="btn btn-sm btn-info" href="admin/iplay_game/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">编辑</a>')
        }else{
            template.push('<a class="btn btn-sm btn-info" href="admin/iplay_game/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">查看</a>')
        }
        if(canDelete=='true'){
            template.push('<a class="btn btn-sm btn-danger" href="javascript:{}" onclick="deleteItem(\'iplay_game\',\'{id}\')">删除</a>')
        }
        template.push('</td>')
        template.push('</tr>')
    ...
    var url ='admin/iplay_game/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
            ...
            $.each(data.info, function(index, val){
                ...
                if(index == "pageList"){
                    ...
                    $.each(val, function(index1, value){
                        ...
                        value.CTX = CTX;
                        //自定义输出
                        var freshTypeMap = {"":"全部","0":"随机"}
                        value.freshType = buildSelect(freshTypeMap,value.freshType,'iplay_game','freshType','id')
                        //聚焦
...
function exportFile(exportFile){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/iplay_game/export/'+exportFile+'/'+start+'/'+limit;
