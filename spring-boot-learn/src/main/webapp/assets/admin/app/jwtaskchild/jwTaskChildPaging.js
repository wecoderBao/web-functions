...
function buildReqParam(){
    var requestParam = {date:new Date().getTime()}
        setRequestParamByName(requestParam,'taskTypeFirst')
...
function  getItemPage(pageIndex){
    var template =
            ...
            '<td><a title="点击查看详情" target="_blank" onclick="openUrl(this,\'admin/jw_task/detail_param?id={jwTaskId}\')"> {jwTaskIdForeignShowValue}</a></td>' +
                    ...
                    '<td>{imgUrl}</td>' +
        '<td>' +
        '<a href="admin/jw_task_child/detail/{id}">编辑</a>' +
        '<a href="javascript:{}" onclick="deleteItem(\'jw_task_child\',{id})">删除</a>'+
        '</td></tr>';
    var pageSize = 20;//每页多少条记录
    var pageCount = parseInt($("#pageCount").val());//总共多少条记录
    var url ='admin/jw_task_child/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
            ...
            $.each(data.info, function(index, val){
                ...
                if(index == "pageList"){
                    ...
                    $.each(val, function(index1, value){
                        ...
                        value.CTX = CTX;
                        //自定义输出
                        var taskTypeMap = {"":"全部","0":"日常","1":"悬赏"}
                        value.taskType = buildSelect(taskTypeMap,value.taskType,'jw_task_child','taskType','id')
                        //赋值替换
                        var tm = template;
...
function exportFile(exportFile){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/jw_task_child/export/'+exportFile+'/'+start+'/'+limit;
