...
function buildReqParam(){
    var requestParam = {date:new Date().getTime()}
        setRequestParamByName(requestParam,'roleIdFirst')
        setRequestParamByName(requestParam,'moduleIdFirst')
        setRequestParamByName(requestParam,'priorityIdFirst')
...
function  getItemPage(){
    ...
    var template = []
        ...
        template.push('<td><a title="点击查看详情" target="_blank" onclick="openUrl(this,\'admin/role/detail_param?id={roleId}\')"> {roleIdForeignShowValue}</a></td>')
        template.push('<td><a title="点击查看详情" target="_blank" onclick="openUrl(this,\'admin/module/detail_param?id={moduleId}\')"> {moduleIdForeignShowValue}</a></td>')
        template.push('<td><a title="点击查看详情" target="_blank" onclick="openUrl(this,\'admin/priority/detail_param?id={priorityId}\')"> {priorityIdForeignShowValue}</a></td>')

        template.push('<td>')
        if(canUpdate=='true'){
            template.push('<a class="btn btn-sm btn-info" href="admin/role_module_priority/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">编辑</a>')
        }else{
            template.push('<a class="btn btn-sm btn-info" href="admin/role_module_priority/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">查看</a>')
        }
        if(canDelete=='true'){
            template.push('<a class="btn btn-sm btn-danger" href="javascript:{}" onclick="deleteItem(\'role_module_priority\',\'{id}\')">删除</a>')
        }
        template.push('</td>')
        template.push('</tr>')
...
function exportFile(exportFile){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/role_module_priority/export/'+exportFile+'/'+start+'/'+limit;
