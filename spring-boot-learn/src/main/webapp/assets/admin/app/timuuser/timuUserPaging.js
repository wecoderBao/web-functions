...
function buildReqParam(){
    var requestParam = {date:new Date().getTime()}
        setRequestParamByName(requestParam,'userFromFirst')
        setRequestParamByName(requestParam,'userKeyFirst')
    setRequestParamByName(requestParam,'passAllFirst')
...
function  getItemPage(){
    ...
    var template = []
        ...
        template.push('<td>{userFrom}</td>')
        template.push('<td>{userKey}</td>')
        template.push('<td>{passAll}</td>')
        ...
        template.push('<td>')
        if(canUpdate=='true'){
            template.push('<a class="btn btn-sm btn-info" href="admin/timu_user/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">编辑</a>')
        }else{
            template.push('<a class="btn btn-sm btn-info" href="admin/timu_user/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">查看</a>')
        }
        if(canDelete=='true'){
            template.push('<a class="btn btn-sm btn-danger" href="javascript:{}" onclick="deleteItem(\'timu_user\',\'{id}\')">删除</a>')
        }
        template.push('</td>')
        template.push('</tr>')
    ...
    var url ='admin/timu_user/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
            ...
            $.each(data.info, function(index, val){
                ...
                if(index == "pageList"){
                    ...
                    $.each(val, function(index1, value){
                        ...
                        value.CTX = CTX;
                        //自定义输出
                        var userFromMap = {"":"全部","0":"爱玩APP","1":"微信","2":"新闻客户端"}
                        value.userFrom = buildSelect(userFromMap,value.userFrom,'timu_user','userFrom','id')
                        var passAllMap = {"":"全部","false":"否","true":"是"}
                        value.passAll = buildSelect(passAllMap,value.passAll,'timu_user','passAll','id')
                        //聚焦
...
function exportFile(exportFile){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/timu_user/export/'+exportFile+'/'+start+'/'+limit;
