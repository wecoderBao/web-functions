...
function buildReqParam(){
    var requestParam = {date:new Date().getTime()}
        setRequestParamByName(requestParam,'userKeyFirst')
    setRequestParamByName(requestParam,'correctFirst')
...
function  getItemPage(){
    ...
    var template = []
        ...
        template.push('<td>{userKey}</td>')
        template.push('<td>{paperSubjectIndex}</td>')
        template.push('<td><a title="点击查看详情" target="_blank" onclick="openUrl(this,\'admin/paper_subject/detail_param?id={paperSubjectId}\')"> {paperSubjectIdForeignShowValue}</a></td>')
        template.push('<td>{answer}</td>')
        template.push('<td>{correct}</td>')
        ...
        template.push('<td>')
        if(canUpdate=='true'){
            template.push('<a class="btn btn-sm btn-info" href="admin/paper_user_subject/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">编辑</a>')
        }else{
            template.push('<a class="btn btn-sm btn-info" href="admin/paper_user_subject/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">查看</a>')
        }
        if(canDelete=='true'){
            template.push('<a class="btn btn-sm btn-danger" href="javascript:{}" onclick="deleteItem(\'paper_user_subject\',\'{id}\')">删除</a>')
        }
        template.push('</td>')
        template.push('</tr>')
    ...
    var url ='admin/paper_user_subject/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
            ...
            $.each(data.info, function(index, val){
                ...
                if(index == "pageList"){
                    ...
                    $.each(val, function(index1, value){
                        ...
                        value.CTX = CTX;
                        //自定义输出
                        var correctMap = {"":"全部","false":"否","true":"是"}
                        value.correct = buildSelect(correctMap,value.correct,'paper_user_subject','correct','id')
                        //聚焦
...
function exportFile(exportFile){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/paper_user_subject/export/'+exportFile+'/'+start+'/'+limit;
