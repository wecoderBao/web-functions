...
function  getItemPage(){
    ...
    var template = []
        ...
        template.push('<td>{totalQty}</td>')
        template.push('<td>{qty}</td>')
        template.push('<td>{userGetNum}</td>')
        ...
        template.push('<td>')
        if(canUpdate=='true'){
            template.push('<a class="btn btn-sm btn-info" href="admin/card_award/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">编辑</a>')
        }else{
            template.push('<a class="btn btn-sm btn-info" href="admin/card_award/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">查看</a>')
        }
        if(canDelete=='true'){
            template.push('<a class="btn btn-sm btn-danger" href="javascript:{}" onclick="deleteItem(\'card_award\',\'{id}\')">删除</a>')
        }
        template.push('</td>')
        template.push('</tr>')
    ...
    var url ='admin/card_award/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
            ...
            $.each(data.info, function(index, val){
                ...
                if(index == "pageList"){
                    ...
                    $.each(val, function(index1, value){
                        ...
                        value.open = buildSelect(openMap,value.open,'card_award','open','id')
                        //聚焦
...
function exportFile(exportFile){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/card_award/export/'+exportFile+'/'+start+'/'+limit;
