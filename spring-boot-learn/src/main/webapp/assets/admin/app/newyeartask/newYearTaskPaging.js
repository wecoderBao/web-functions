...
function buildReqParam(){
    var requestParam = {date:new Date().getTime()}
        setRequestParamByName(requestParam,'taskNameFirst')
        setRequestParamByName(requestParam,'taskCategoryFirst')
...
function  getItemPage(){
    ...
    var template = []
        ...
        template.push('<td>{taskName}</td>')
        template.push('<td>{taskCategory}</td>')
        template.push('<td>{finishTaskAward}</td>')
        template.push('<td>{notFinishTaskAward}</td>')
        template.push('<td>{finishNeedCount}</td>')
        template.push('<td>{luckTask}</td>')
        ...
        template.push('<td>{step}</td>')

        template.push('<td>')
        if(canUpdate=='true'){
            template.push('<a class="btn btn-sm btn-info" href="admin/new_year_task/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">编辑</a>')
        }else{
            template.push('<a class="btn btn-sm btn-info" href="admin/new_year_task/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">查看</a>')
        }
        if(canDelete=='true'){
            template.push('<a class="btn btn-sm btn-danger" href="javascript:{}" onclick="deleteItem(\'new_year_task\',\'{id}\')">删除</a>')
        }
        template.push('</td>')
        template.push('</tr>')
    ...
    var url ='admin/new_year_task/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
            ...
            $.each(data.info, function(index, val){
                ...
                if(index == "pageList"){
                    ...
                    $.each(val, function(index1, value){
                        ...
                        value.CTX = CTX;
                        //自定义输出
                        var taskCategoryMap = {"":"全部","0":"立即获取","1":"订阅栏目","2":"分享活动","3":"邀请好友","4":"阅读资讯","5":"分享资讯"}
                        value.taskCategory = buildSelect(taskCategoryMap,value.taskCategory,'new_year_task','taskCategory','id')
                        var luckTaskMap = {"":"全部","false":"否","true":"是"}
                        value.luckTask = buildSelect(luckTaskMap,value.luckTask,'new_year_task','luckTask','id')
                        //聚焦
...
function exportFile(exportFile){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/new_year_task/export/'+exportFile+'/'+start+'/'+limit;
