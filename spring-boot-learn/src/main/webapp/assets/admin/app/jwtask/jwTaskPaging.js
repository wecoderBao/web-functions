...
function  getItemPage(pageIndex){
    var template =
            '<tr id="tr{id}"><td><input type="checkbox" value="{id}" class="tdcheckbox"></td><td>{rankNum}</td>'+
                    ...
                    '<td>{id}</td>' +
                    '<td>{name}</td>' +
                    '<td>{subtractCreditOnePrizeChance}</td>' +
                    '<td>{open}</td>' +
        '<td>' +
        '<a href="admin/jw_task/detail/{id}">编辑</a>' +
        '<a href="javascript:{}" onclick="deleteItem(\'jw_task\',{id})">删除</a>'+
        '</td></tr>';
    var pageSize = 20;//每页多少条记录
    var pageCount = parseInt($("#pageCount").val());//总共多少条记录
    var url ='admin/jw_task/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
            ...
            $.each(data.info, function(index, val){
                ...
                if(index == "pageList"){
                    ...
                    $.each(val, function(index1, value){
                        ...
                        value.open = buildSelect(openMap,value.open,'jw_task','open','id')
                        //赋值替换
                        var tm = template;
...
function exportFile(exportFile){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/jw_task/export/'+exportFile+'/'+start+'/'+limit;
