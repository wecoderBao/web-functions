...
function  getItemPage(pageIndex){
    var template =
            '<tr id="tr{id}"><td><input type="checkbox" value="{id}" class="tdcheckbox"></td><td>{rankNum}</td>'+
                    ...
                    '<td>{credit}</td>' +
        '<td>' +
        '<a href="admin/jw_user_credit/detail/{id}">编辑</a>' +
        '<a href="javascript:{}" onclick="deleteItem(\'jw_user_credit\',{id})">删除</a>'+
        '</td></tr>';
    var pageSize = 20;//每页多少条记录
    var pageCount = parseInt($("#pageCount").val());//总共多少条记录
    var url ='admin/jw_user_credit/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
...
function exportFile(exportFile){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/jw_user_credit/export/'+exportFile+'/'+start+'/'+limit;
