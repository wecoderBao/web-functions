...
function buildReqParam(){
    var requestParam = {date:new Date().getTime()}
        ...
        setRequestParamByName(requestParam,'userNameFirst')
        setRequestParamByName(requestParam,'remindMessageFirst')
        setRequestParamByName(requestParam,'sendFirst')
    setRequestParamByName(requestParam,'createTimeFirst')
    setRequestParamByName(requestParam,'createTimeSecond')
    setRequestParamByName(requestParam,'recomendFirst')
        setRequestParamByName(requestParam,'prizeRandCodeFirst')
...
function  getItemPage(){
    ...
    var template = []
        ...
        template.push('<td>{id}</td>')
        template.push('<td>{prizeRandCode}</td>')
        template.push('<td>{userPhone}</td>')
        ...
        template.push('<td><a title="点击查看详情" target="_blank" onclick="openUrl(this,\'admin/prize_detail/detail_param?id={prizeDetailId}\')"> {prizeDetailIdForeignShowValue}</a></td>')
        template.push('<td>{userId}</td>')
        template.push('<td>{userName}</td>')
        template.push('<td>{remindMessage}</td>')
        template.push('<td>{send}</td>')
        template.push('<td>{createTime}</td>')
        template.push('<td>{recomend}</td>')

        template.push('<td>')
        if(canUpdate=='true'){
            template.push('<a class="btn btn-sm btn-info" href="admin/prize_user_win/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">编辑</a>')
        }else{
            template.push('<a class="btn btn-sm btn-info" href="admin/prize_user_win/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">查看</a>')
        }
        if(canDelete=='true'){
            template.push('<a class="btn btn-sm btn-danger" href="javascript:{}" onclick="deleteItem(\'prize_user_win\',\'{id}\')">删除</a>')
        }
        template.push('</td>')
        template.push('</tr>')
    ...
    var url ='admin/prize_user_win/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
            ...
            $.each(data.info, function(index, val){
                ...
                if(index == "pageList"){
                    ...
                    $.each(val, function(index1, value){
                        ...
                        value.CTX = CTX;
                        //自定义输出
                        var sendMap = {"":"全部","0":"未发放","1":"已发放"}
                        value.send = buildSelect(sendMap,value.send,'prize_user_win','send','id')
                        var recomendMap = {"":"全部","false":"否","true":"是"}
                        value.recomend = buildSelect(recomendMap,value.recomend,'prize_user_win','recomend','id')
                        //聚焦
...
function exportFile(exportFile){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/prize_user_win/export/'+exportFile+'/'+start+'/'+limit;
