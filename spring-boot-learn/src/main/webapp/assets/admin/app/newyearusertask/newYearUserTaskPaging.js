...
function buildReqParam(){
    var requestParam = {date:new Date().getTime()}
        ...
        setRequestParamByName(requestParam,'newYearTaskIdFirst')
...
function  getItemPage(){
    ...
    var template = []
        ...
        template.push('<td><a title="点击查看详情" target="_blank" onclick="openUrl(this,\'admin/new_year_task/detail_param?id={newYearTaskId}\')"> {newYearTaskIdForeignShowValue}</a></td>')
        template.push('<td>{finish}</td>')
        template.push('<td>{hasFinishNeedCount}</td>')
        template.push('<td>{createTime}</td>')

        template.push('<td>')
        if(canUpdate=='true'){
            template.push('<a class="btn btn-sm btn-info" href="admin/new_year_user_task/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">编辑</a>')
        }else{
            template.push('<a class="btn btn-sm btn-info" href="admin/new_year_user_task/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">查看</a>')
        }
        if(canDelete=='true'){
            template.push('<a class="btn btn-sm btn-danger" href="javascript:{}" onclick="deleteItem(\'new_year_user_task\',\'{id}\')">删除</a>')
        }
        template.push('</td>')
        template.push('</tr>')
    ...
    var url ='admin/new_year_user_task/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
            ...
            $.each(data.info, function(index, val){
                ...
                if(index == "pageList"){
                    ...
                    $.each(val, function(index1, value){
                        ...
                        value.CTX = CTX;
                        //自定义输出
                        var finishMap = {"":"全部","false":"否","true":"是"}
                        value.finish = buildSelect(finishMap,value.finish,'new_year_user_task','finish','id')
                        //聚焦
...
function exportFile(exportFile){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/new_year_user_task/export/'+exportFile+'/'+start+'/'+limit;
