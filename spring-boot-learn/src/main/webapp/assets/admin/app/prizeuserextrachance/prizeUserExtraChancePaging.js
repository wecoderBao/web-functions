...
function  getItemPage(pageIndex){
    var template =
            '<tr id="tr{id}"><td><input type="checkbox" value="{id}" class="tdcheckbox"></td><td>{rankNum}</td>'+
                    ...
                    '<td>{userId}</td>' +
        '<td>' +
        '<a href="admin/prize_user_extra_chance/detail/{id}">编辑</a>' +
        '<a href="javascript:{}" onclick="deleteItem(\'prize_user_extra_chance\',{id})">删除</a>'+
        '</td></tr>';
    var pageSize = 20;//每页多少条记录
    var pageCount = parseInt($("#pageCount").val());//总共多少条记录
    var url ='admin/prize_user_extra_chance/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
...
function exportFile(exportFile){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/prize_user_extra_chance/export/'+exportFile+'/'+start+'/'+limit;
