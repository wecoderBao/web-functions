...
function buildReqParam(){
    var requestParam = {date:new Date().getTime()}
        ...
        setRequestParamByName(requestParam,'battleTagFirst')
...
function  getItemPage(){
    ...
    var template = []
        ...
        template.push('<td>{battleTag}</td>')
        template.push('<td>{mobile}</td>')
        template.push('<td>{address}</td>')

        template.push('<td>')
        if(canUpdate=='true'){
            template.push('<a class="btn btn-sm btn-info" href="admin/d3_address/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">编辑</a>')
        }else{
            template.push('<a class="btn btn-sm btn-info" href="admin/d3_address/detail/{id}?queryParamCacheKey='+queryParamCacheKey+'">查看</a>')
        }
        if(canDelete=='true'){
            template.push('<a class="btn btn-sm btn-danger" href="javascript:{}" onclick="deleteItem(\'d3_address\',\'{id}\')">删除</a>')
        }
        template.push('</td>')
        template.push('</tr>')
    ...
    var url ='admin/d3_address/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
...
function exportFile(exportFile){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/d3_address/export/'+exportFile+'/'+start+'/'+limit;
