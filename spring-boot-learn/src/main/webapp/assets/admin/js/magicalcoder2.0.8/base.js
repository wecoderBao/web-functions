...
function uploadFile2(index,folder,_t){
    var img = $(_t).parent().find("img")
    var inputSrc = $(_t).parent().find('input[name="'+index+'"]')
    $.ajaxFileUpload({
        //处理文件上传操作的服务器端地址(可以传参数)
        ...
        success:function(data, status){        //服务器响应成功时的处理函数
            if(data.code==0){     //0表示上传成功(后跟上传后的文件路径),1表示失败(后跟失败描述)
                var uploadSrc = data.url
                var uploadUrl=uploadSrc;
                if(uploadSrc!=''){
                    var arr = uploadSrc.split("//")
                    if(arr[0]!='http:'&& arr[0]!='https:'){
                        uploadUrl = CTX+uploadSrc;
                    }
                }

                img.attr("src", uploadUrl);
                inputSrc.val(uploadSrc)
            }else{
                alert(data.message)
            }
        },
...
function randomStr(length) {
    ...
    for (var i = 0; i < length; i++) {
        var idx = parseInt(Math.random()*(base.length),10);
        result +=base.charAt(idx);
    }
    return result
}
//============================表单验证===========================
...
function validateForm2($formJquery){
    $formJquery.find("input").on('keyup blur', function() {
        var _t = $(this);
        validateItem(_t)
    })
...
function submitForm(formJquery,callback){
    $(formJquery).find("input").each(function(){
        var _t = $(this);
        if(_t.attr("type")=='text'){
            _t.val($.trim(_t.val()))
            validateItem(_t)
        }
    })
    $(formJquery).find("textarea").each(function() {
        var _t = $(this);
        _t.val($.trim(_t.val()))
        validateItem(_t)
    })
...
//提交验证
function submitForm2(validateArea,formJquery,callback){
    $(validateArea).find("input").each(function(){
        var _t = $(this);
        if(_t.attr("type")=='text'){
            _t.val($.trim(_t.val()))
            validateItem(_t)
    ...
    $(validateArea).find("textarea").each(function() {
        var _t = $(this);
        _t.val($.trim(_t.val()))
        validateItem(_t)
    })
    //提交表单
    var length = $(validateArea).find(".has-error").length
    if(length<=0){
        if(callback){
            callback.call()
        }else{
