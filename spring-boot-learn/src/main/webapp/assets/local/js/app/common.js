			...
			preloadImg: function() {
				...
				if (loadList.length) {
					fn.imgLoader.init(loadList, function() {
						...
						setTimeout(function(){
							$(".page-1").addClass("hide").next().removeClass("hide");
						},500);
					}, function(percent) {
						Loadmark.find('.value').text(percent + '%');
						Loadmark.find('i').width(percent + '%');
						$(".page-1 .wrap").css({left:percent-20+"%"});
					}).load();
				} else {
					Loadmark.remove();
...
doAjax.prototype.ajaxError = function(){
	console.log("error")
};
doAjax.prototype.http = function(url, data, success, error){
	error = typeof error === "function" ? error : this.ajaxError;
	var _index = 0;
	$.each(data, function(n, v) {
		if (_index == 0) {
...
//var debug = false;
var debug = true,jiekou,activeUrl;
if(debug){
	jiekou = "http://localhost.163.com/zoo/zookeeper";
	activeUrl = "dd"
}else{
	jiekou = "http://app.game.163.com/zoo/zookeeper";
	activeUrl = ''
}
var activeInfo = new doAjax(jiekou);
var huoDongId = 5;
