...
function zTreeOnDblClick(event, treeId, treeNode) {
	...
	if(!(treeNode.getParentNode()==null && treeNode.id==0)){
		ModuleWindow_get(treeNode).showAlert();
	}
};
...
//	alert("删除节点成功 在此处ajax删除数据库此节点即可");
	ModuleEdit_get().delNode(treeNode);
}
...
function onRename(e, treeId, treeNode) {
	ModuleEdit_get().renameNode(treeNode);
...
//					+ " 在此处ajax保存到数据库返回id后更新id");
			ModuleEdit_get().addNode(newNodeJson);
			return false;
		});
};
...
$(document).ready(function() {
	ModuleEdit_get().initData(setting);
});
function submit(){
	ModuleEdit_get().submit();
}
